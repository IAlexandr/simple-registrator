...
function move({ filename, folder, sessionId}, callback) {
  db.Segment.create().then((segment) => {
    ...
    console.log(readFilePath, ' writing to ', fileStoreKey);
    ...
    rs.on('end', () => {
      ...
      setTimeout(() => {
        store.exists({key: fileStoreKey}, (err, isExists) => {
          console.log(fileStoreKey, isExists);
        });
      }, 500);
  ...
  fs.watch(folder, (eventType, filename) => {
    switch (eventType) {
      case 'rename':
        ...
        if (filename === 'manifest.mpd' && !session.mpdHeader) {
          manifestParser(filePath, (err, mpdJSON) => {
            if (err) {
              debug('err:', err.message);
            } else {
              session.mpdHeader = mpdJSON;
            }
          });
        } else if (!filename.match(/.tmp/g) && filename !== 'manifest.mpd' && filename !== 'init-stream0.m4s') {
          const filePath = path.resolve(folder, filename);
